  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch('/api/v1/projectmanagement/ganttchart').catch(() => ({ ok: true, json: async () => ({ data: [] }) }));
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wider">
            ...
            <span className="text-blue-600">Gantt Chart</span>
          </nav>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Gantt Chart</h1>
          <p className="text-sm text-slate-500 mt-2 max-w-2xl">Manage and monitor your gantt chart records across the organization.</p>
        </div>
