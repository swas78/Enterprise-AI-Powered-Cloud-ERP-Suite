  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch('/api/v1/ai/modelstatus').catch(() => ({ ok: true, json: async () => ({ data: [] }) }));
    ...
    <div className="p-6 w-full max-w-7xl mx-auto">
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">Model Status</h1>
          <p className="text-sm text-slate-500 mt-2 max-w-2xl">Manage and monitor your model status records across the organization.</p>
        </div>
