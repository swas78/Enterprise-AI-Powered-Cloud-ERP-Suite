...
const genericTemplate = (componentName, folderName) => `import React, { useState, useEffect } from 'react';
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await apiFetch('/api/v1/${folderName.toLowerCase()}/${componentName.toLowerCase()}').catch(() => ({ ok: true, json: async () => ({ data: [] }) }));
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-4">
        <div>
          <nav className="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-2 uppercase tracking-wider">
            ...
            <span className="text-blue-600">${componentName.replace(/([A-Z])/g, ' $1').trim()}</span>
          </nav>
          <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">${componentName.replace(/([A-Z])/g, ' $1').trim()}</h1>
          <p className="text-sm text-slate-500 mt-2 max-w-2xl">Manage and monitor your ${componentName.replace(/([A-Z])/g, ' $1').trim().toLowerCase()} records across the organization.</p>
        </div>
...
files.forEach(file => {
  if (file.endsWith('.tsx')) {
    ...
    if (content.includes('scaffolded') || content.includes('stub')) {
      const fileName = path.basename(file, '.tsx');
      // Get folder name (e.g. Finance, HR)
      const folderName = path.basename(path.dirname(file));
      
      const newContent = genericTemplate(fileName, folderName);
...
console.log('Successfully replaced ' + replacedCount + ' scaffolded files with functional components!');
